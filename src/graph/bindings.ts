...
export function resolveRecvType(
  ...
  // C++ out-of-class definition bodies (`void Physics::step() { … }`) walk with
  // scope ["Physics.step"], not ["Physics", "step"], so a same-file field
  // binding stored under the class's OWN scope ("Physics") needs this second
  // probe — the lookup above only ever tries "Physics.step" then "".
  if (ctx.lang === "cpp" && ctx.enclosingClass) {
    return ctx.bindings.lookup([ctx.enclosingClass], receiver) ?? undefined;
  }
  return undefined;
}
