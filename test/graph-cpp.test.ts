/**
 * Tests for C/C++ extraction in the Tier-1 code graph — a hand-written depth
 * extractor rather than the generic (breadth-tier) tags.scm walk, so member
 * calls can be typed instead of resolved on bare names alone.
 *
 ...
 * reference edges, and call edges exist only where the conservative resolver
 * can commit (pinned in graph-cpp-edges.test.ts) — everything else is dropped
 * rather than guessed.
 */
...
test("C/C++ extraction: functions, methods, classes, structs, enums", async () => {
  ...
  // commit — here, the same-file `tick` → `snapEntityToFloor` call.
