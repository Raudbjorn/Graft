/**
 * C++ receiver-type bindings: `Body body;`, parameter types (`Entity* e`),
 * and same-file class fields bind variable → type, so member calls resolve
 * through the owner-qualified index exactly like Python/Go receivers do —
 * same-file → extracted, cross-file → inferred, ambiguous or untyped →
 * dropped. `auto` binds nothing (no inference).
 */
